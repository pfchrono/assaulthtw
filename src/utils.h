...
#define TRANSPORT_VEHICLE(type)	(type == VEHICLE_TRUCK || type == VEHICLE_CHINOOK || type == VEHICLE_CARRIER)
#define AIR_VEHICLE(type)	(type == VEHICLE_AIRCRAFT || type == VEHICLE_CHINOOK || type == VEHICLE_BOMBER || type == VEHICLE_BIO_FLOATER || type == VEHICLE_RECON )
#define continual_flight(vhc)	(AIR_VEHICLE(vhc->type) && vhc->type != VEHICLE_CHINOOK)
#define HAS_INT_DEFENSE(bld)	((build_table[bld->type].military == TRUE && bld->value[3] >= 0 && bld->security >0))
#define IS_AWAKE(ch)            (ch->position > POS_SLEEPING)
#define WAIT_STATE(ch, npulse)  ((ch)->wait = UMAX((ch)->wait, (npulse)))
#define IS_BUGGED_AREA(x,y)	((x<0))
#define paintball(ch)		((ch->z == Z_PAINTBALL && ch->x >= 200 && ch->y >= 200 && ch->x <= 300 && ch->y <= 300))
#define INVALID_COORDS(x,y)	(0)
...
#define WAREHOUSE(bld)		((bld->type == BUILDING_WAREHOUSE ))
/*
 * Description macros.
 */
