...
void	talk_channel	args( ( CHAR_DATA *ch, char *argument,
			    int channel, const char *verb, const char *blind_verb ) );

/*
 * Generic channel function.
 */

void talk_channel( CHAR_DATA *ch, char *argument, int channel, const char *verb, const char *blind_verb )
{
...
//	send_to_char( "Please use the Newbie channel for now.\n\r", ch );
   ...
   switch ( channel )
	...
	if ( ( IS_SET(ch->config, CONFIG_ECHAN ) ) && !IS_SET(ch->act, PLR_WIZINVIS ) )
        ...
        sprintf( blind_buf, "%s %s%s%s $t@@N", blind_verb, (fake)?"Someone":"$n", (sysdata.pikamod)?"Mon":"", gemote ? "" : ":" );
  ...
  case CHANNEL_ALLIANCE:
	if ( ( IS_SET(ch->config, CONFIG_ECHAN ) ) && !IS_SET(ch->act, PLR_WIZINVIS ) )
        ...
        sprintf( blind_buf, "%s $n%s $t@@N", blind_verb, gemote ? "" : ":" );
        act( buf, ch, argument, NULL, TO_CHAR );
        break;
   ...
   }
   {
	...
	&&  !IS_SET(och->deaf, CHANNEL_HERMIT) )
	{
		...
		if ( IS_SET(vch->config,CONFIG_BLIND) )
        	  	sprintf( ansi, "%s", blind_buf );
		else
	          	sprintf( ansi, "%s", buf );
...
{
   talk_channel( ch, argument, CHANNEL_CREATOR, "@@d:@@g:@@rD@@GR@@d-@@aN@@cet@@g:@@d:@@N", "Creator:" );
   return;
}
...
void do_gossip( CHAR_DATA *ch, char *argument )
{
    ...
    talk_channel( ch, argument, CHANNEL_GOSSIP, "@@G[@@rGO@@WSS@@rIP@@G]@@N", "Gossip");
    return;
}
...
void do_game( CHAR_DATA *ch, char *argument )
{
	...
	if ( guess_pause > 0 )
		mreturn("Please, don't use triggers for this.\n\r", ch );
	if ( g > 0 && g <= 1000 )
	{
...
void do_newbie( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	talk_channel( ch, "Important: I am gay, and Dragonball: Saga Mud sucks! You all rule!", CHANNEL_NEWBIE, "@@2@@W[@@1@@WNEWBIE@@2@@W]@@N", "newbie" );
    ...
    smash_swear(argument);
    talk_channel( ch, argument, CHANNEL_NEWBIE, "@@2@@W[@@1@@WNEWBIE@@2@@W]@@N", "Newbie" );
    return;
}
...
void do_atalk( CHAR_DATA *ch, char *argument )
{
    ...
    talk_channel( ch, argument, CHANNEL_ALLIANCE, buf, alliance_table[ch->pcdata->alliance].name );
    if ( get_trust(ch) >= 84 )
...
{
    talk_channel( ch, argument, CHANNEL_QUESTION, "@@l[@@cQUESTION@@l]@@N", "Question" );
    return;
}
...
{
    talk_channel( ch, argument, CHANNEL_QUESTION, "@@c[@@lANSWER@@c]@@N", "Answer" );
    return;
}
...
void do_flame( CHAR_DATA *ch, char *argument )
{
    ...
    smash_swear(argument);
    talk_channel( ch, argument, CHANNEL_FLAME, "@@e[@@RF@@eL@@RA@@eM@@RE@@e]@@N", "Flame" );
    return;
}
...
void do_immtalk( CHAR_DATA *ch, char *argument )
{
  ...
  talk_channel( ch, argument, CHANNEL_IMMTALK, "immtalk", "immtalk" );  
  return;
}
...
void do_ooc( CHAR_DATA *ch, char *argument )
{
    ...
    smash_swear(argument);
    talk_channel( ch, argument, CHANNEL_OOC, "@@d[@@lO@@BO@@lC@@d]@@N", "ooc" );
    return;
}
void do_politics( CHAR_DATA *ch, char *argument )
{
    smash_swear(argument);
    talk_channel( ch, argument, CHANNEL_POLITICS, "@@d[@@eP@@RO@@eL@@d]@@N", "politics" );
    return;
}
...
void do_tell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->desc )
    {
    	sprintf( buf, "%s%s$n%s tells you '%s$t%s'.", (victim->desc->mxp) ? "\e[1z" : "", MXPTAG(victim->desc,"player $n"), MXPTAG(victim->desc,"/player"), color_string( victim, "tell" ), color_string( victim, "normal" ) );
...
void do_quit( CHAR_DATA *ch, char *argument )
{
    ...
    {
	int x,y,xx,yy;
	for ( xx=ch->x-1;xx<ch->x+1;xx++ )
	for ( yy=ch->y-1;yy<ch->y+1;yy++ )
	{
		x=xx;y=yy;real_coords(&x,&y);
		if ((bld = map_bld[x][y][ch->z]) == NULL) continue;
		if ( !bld->active ) mreturn("You can't log off next to offline buildings.\n\r", ch );
	    ...
	    if ( ch->in_building && ch->in_building->owner != ch )
	    {
